...
from .commands.evaluation_commands_refactored import (
    EvaluationCommands as EvaluationCommandsRefactored,
)
...
class ModernCLIApp(CLIApplication):
    ...
    def run(self, args: Optional[List[str]] = None) -> int:
        ...
        try:
            ...
            all_params, validation_error = self.parser.extract_and_validate_params(
                parsed_args
            )
            if validation_error:
            ...
            return self._execute_operation_with_error_handling(
                parsed_args, all_params, operation_type
            )
    ...
    def _execute_operation_with_error_handling(
        self, args, all_params: dict, operation_type: str
    ) -> int:
    ...
    def _handle_list_snapshots(self, args) -> int:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.io.print_error(
                    f"Failed to list snapshots: HTTP {response.status_code}"
                )
                return 1
    ...
    def _handle_export_snapshot(self, args) -> int:
        ...
        if not task_id or not label:
            self.io.print_error(
                "--task-id and --label are required for --export-snapshot"
            )
            return 1
        ...
        try:
            ...
            response = requests.get(
                f"{base_url}/tasks/{task_id}/context/snapshots/{label}"
            )
            ...
            if response.status_code == 200:
                ...
                self.io.print_error(
                    f"Failed to export snapshot: HTTP {response.status_code}"
                )
                return 1
    ...
    def _handle_index_run_root(self, args) -> int:
        ...
        try:
            ...
            self.io.print_success(
                f"INDEX.md regenerated at {path_written}. History updated."
            )
            return 0
    ...
    def _handle_move_task(self, args) -> int:
        ...
        try:
            ...
            parent_desc = (
                "root level" if new_parent_id is None else f"under task {new_parent_id}"
            )
    ...
    def _handle_generate_embeddings(self, args) -> int:
        ...
        try:
            ...
            result = service.precompute_embeddings_for_completed_tasks(
    ...
    def _handle_rebuild_embeddings(self, args) -> int:
        ...
        if not self.io.confirm(
            "This will rebuild ALL embeddings. Continue?", default=False
        ):
        ...
        try:
            ...
            self.io.print_info(
                f"Rebuilding all embeddings (batch size: {batch_size})..."
            )

            # This would need to be implemented in the embeddings service
            # For now, just show what we would do
            self.io.print_info(
                "Rebuild embeddings not yet implemented - would clear cache and regenerate all"
            )
            return 0
    ...
    def _handle_benchmark(self, args) -> int:
        ...
        try:
            ...
            if not configs or not isinstance(configs, list):
                self.io.print_error(
                    "--benchmark-configs is required (one or more specs)"
                )
                return 1
    ...
    def _show_help_guidance(self) -> int:
        ...
        self.io.print_info(
            "This CLI helps you plan and execute projects with LLM assistance."
        )
        self.io.print_info("")
        self.io.print_info("Common operations:")
        self.io.print_info(
            "  --goal 'Your goal here'           Create and execute a new plan"
        )
        self.io.print_info(
            "  --list-plans                      List all existing plans"
        )
        self.io.print_info(
            "  --execute-only --title 'Plan'     Execute an existing plan"
        )
        self.io.print_info("  --rerun-task 123                  Rerun a specific task")
        self.io.print_info(
            "  --rerun-interactive --title 'Plan' Interactive task rerun"
        )
        self.io.print_info("")
