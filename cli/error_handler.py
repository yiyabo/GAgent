...
class CLIErrorHandler:
    ...
    def _handle_custom_error(self, error: BaseError) -> CLIErrorInfo:
        ...
        # 构建用户友好的建议
        suggestions = self._build_user_friendly_suggestions(error, message_data.get("suggestions", []))
        ...
        if isinstance(error, KeyboardInterrupt):
            ...
            return CLIErrorInfo(
                ...
                message=(
                    ...
                    else f"File not found: {error_message}"
                ),
                suggestions=[
            ...
            return CLIErrorInfo(
                ...
                message=(
                    ...
                    if self.language == Language.ZH_CN
                    else f"Permission denied: {error_message}"
                ),
                suggestions=[
            ...
            return CLIErrorInfo(
                ...
                message=(
                    f"参数值错误: {error_message}"
                    if self.language == Language.ZH_CN
                    else f"Invalid parameter value: {error_message}"
                ),
                suggestions=[
                    ...
                    (
                        "使用 --help 查看正确用法"
                        if self.language == Language.ZH_CN
                        else "Use --help to see correct usage"
                    ),
                ],
                show_help=True,
            )
            ...
            return CLIErrorInfo(
                ...
                message=(
                    ...
                    if self.language == Language.ZH_CN
                    else f"Unknown error ({error_type}): {error_message}"
                ),
                suggestions=[
                    ...
                    (
                        "如问题持续存在，请报告此错误"
                        if self.language == Language.ZH_CN
                        else "Report this error if problem persists"
                    ),
                ],
    ...
    def _build_user_friendly_suggestions(self, error: BaseError, base_suggestions: list) -> list:
        """构建用户友好的建议"""
        suggestions = []
...
def handle_cli_exception(error: Exception, verbose: bool = False, chinese: bool = True) -> int:
    """
    处理CLI异常的便捷函数
...
class CLIErrorContext:
    ...
    def __init__(self, operation_name: str, verbose: bool = False, chinese: bool = True):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_val:
            ...
            if self.handler.language == Language.ZH_CN:
                contextual_message = (
                    f"Error occurred while executing operation '{self.operation_name}':\n{error_info.message}"
                )
            else:
                contextual_message = (
                    f"Error occurred while executing operation '{self.operation_name}':\n{error_info.message}"
