...
class ModularCLIParser:
    ...
    def _validate_cross_handler_dependencies(self, all_params: Dict[str, Any]) -> Optional[str]:
        ...
        needs_goal = (
            core_params.get('plan_only') or 
            (not any([
                database_params,
                any(param in ['eval_stats', 'eval_batch', 'eval_supervision', 'eval_supervision_config'] 
                   for param in all_params.get('evaluation', {})),
                database_params.get('rerun_task') or database_params.get('rerun_subtree'),
                any(param in ['list_plans', 'load_plan'] 
                   for param in all_params.get('plan', {})),
                any(param in ['index_preview', 'index_export', 'index_run_root'] 
