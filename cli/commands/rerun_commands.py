...
class RerunCommands(MultiCommand):
    ...
    def handle_subtree(self, args: Namespace) -> int:
        ...
        return self._rerun_subtree(
            task_id, args.use_context, include_parent, context_options
        )
    ...
    def handle_interactive(self, args: Namespace) -> int:
        ...
        return self._interactive_rerun_tasks(
            repo, args.title, args.use_context, context_options
        )
    ...
    def _rerun_single_task(
        ...
        try:
            ...
            response = requests.post(
                f"http://127.0.0.1:9000/tasks/{task_id}/rerun",
                json=payload,
                timeout=300,
            )
    ...
    def _rerun_subtree(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.io.print_error(
                    f"Subtree rerun failed: HTTP {response.status_code}"
                )
                if response.text:
                    print(f"Error: {response.text}")
                return 1
    ...
    def _interactive_rerun_tasks(
        ...
        try:
            ...
            self.io.print_success(
                f"Successfully rerun {success_count}/{len(selected_tasks)} tasks"
            )
    ...
    def _get_plan_tasks(
        self, repo: SqliteTaskRepository, plan_title: str
    ) -> List[dict]:
    ...
    def _select_tasks_for_rerun(self, tasks: List[dict]) -> List[dict]:
        ...
        if not rerunnable_tasks:
            self.io.print_warning(
                "No rerunnable tasks found (only completed/failed tasks can be rerun)"
            )
            return []
