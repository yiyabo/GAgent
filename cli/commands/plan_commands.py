...
from ..utils import FileUtils, PlanUtils
...
class PlanCommands(MultiCommand):
    ...
    def handle_default(self, args: Namespace) -> int:
        ...
        if not hasattr(args, "goal") or not args.goal:
            ...
            self.io.print_info(
                "Or: --execute-only --title 'Plan Name' to execute existing plan"
            )
            return 1
    ...
    def handle_plan_only(self, args: Namespace) -> int:
        ...
        try:
            ...
            if PlanUtils.save_plan_files(plan_result):
                self.io.print_success(
                    f"Plan saved to {PlanUtils.PLAN_JSON} and {PlanUtils.PLAN_MD}"
                )
                self.io.print_info(
                    "Review and edit the plan, then run without --plan-only to execute"
                )
                return 0
            else:
    ...
    def _create_and_execute_plan(self, args: Namespace) -> int:
        ...
        try:
            ...
            self.io.print_success(
                f"Plan created with {len(plan_result.get('tasks', []))} tasks"
            )
            ...
            if not getattr(args, "yes", False):  # Not auto-approve
                if not getattr(args, "no_open", False):  # Not no-open
                    if self.confirm_action(
                        f"Open {PlanUtils.PLAN_MD} for review/editing?"
                    ):
                        FileUtils.open_in_editor(PlanUtils.PLAN_MD)
                        self.io.safe_input(
                            "Press Enter after reviewing/editing the plan..."
                        )
                        ...
                        if edited_plan and PlanUtils.validate_plan(edited_plan):
                            ...
                            self.io.print_warning(
                                "Using original plan (edited version invalid)"
                            )
            ...
            if not getattr(args, "plan_only", False):
                if getattr(args, "yes", False) or self.confirm_action(
                    ...
                    self.io.print_info(
                        f"Plan saved. Execute later with: --execute-only --title '{title}'"
                    )
