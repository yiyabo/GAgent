    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {"type": "string", "description": "要可视化的计划的标题。"}
                },
                "required": ["title"],
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "title": {
                        "type": "string",
                        "description": "正在执行的计划的完整标题。",
                    },
                    "task_id": {
                        "type": "integer",
                        "description": "要执行的原子任务的ID。",
                    },
                    "output_filename": {
                        "type": "string",
                        "description": "可选：生成内容要保存的文件名（例如 notes.md）",
                    },
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "content": {"type": "string", "description": "待办事项的具体内容。"}
                },
                "required": ["content"],
            },
        },
...
class ChatCommands(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--session", type=str, default=None, help="Session label (optional)"
        )
        parser.add_argument(
            "--max-turns",
            type=int,
            default=0,
            help="Autostop after N turns (0=unlimited)",
        )
        parser.add_argument(
            "--debug", action="store_true", help="Enable debug logging for tool calls"
        )
    ...
    def _execute_impl(self, args: Namespace) -> int:
        ...
        self.io.print_info(
            f"Server: {self.base_url}, Model: {getattr(args, 'model', 'default')}"
        )
        ...
        while True:
            ...
            if user_in.startswith("/"):
                ...
                if cmd == "/clear":
                    ...
                    try:
                        ...
                        pass
                    self.console.print(
                        "[green]Context cleared. Starting fresh.[/green]"
                    )
                    continue
                # Unknown slash command hint
                if cmd not in {"/clear"}:
                    self.console.print(
                        "[yellow]Unknown command. Try /clear or /exit[/yellow]"
                    )
                    continue
            ...
            while True:
                ...
                with self.console.status(
                    "[bold cyan]LLM processing...", spinner="dots"
                ):
                    response = self._call_llm_with_tools(history, args)
                if not response:
                    self.console.print(
                        "[bold red]Error: Failed to get response from LLM.[/bold red]"
                    )
                ...
                if message.get("tool_calls"):
                    history.append(
                        message
                    )  # Add assistant's tool-calling request to history
                    tool_calls = message["tool_calls"]

                    if self.debug_mode:
                        self.console.log(
                            f"LLM requests tool calls: {tool_calls}", style="yellow"
                        )
            ...
            if max_turns > 0 and turns >= max_turns:
                self.console.print(
                    "[bold yellow]Max turns reached. Exiting.[/bold yellow]"
                )
                break
    ...
    def _call_llm_with_tools(
        self, history: List[Dict[str, Any]], args: Namespace
        ...
        try:
            ...
            if not api_key:
                self.console.print(
                    "[bold red]Error: GLM_API_KEY is not set.[/bold red]"
                )
                return None
            ...
            if self.debug_mode:
                self.console.log(
                    f"LLM request to {api_url}: {json.dumps(payload, indent=2, ensure_ascii=False)}",
                    style="bold green",
                )
            ...
            if self.debug_mode:
                self.console.log(
                    f"LLM response: {json.dumps(response_json, indent=2, ensure_ascii=False)}",
                    style="bold blue",
                )
            ...
            self.console.print(
                f"[bold red]LLM API HTTP Error: {e.response.status_code} {e.response.text}[/bold red]"
            )
            return None
        except TimeoutException:
            self.console.print(
                f"[bold red]LLM API call timed out after {timeout_seconds} seconds.[/bold red]"
            )
            return None
    ...
    def _execute_tool_call(
        ...
        try:
            ...
            if tool_name == "execute_atomic_task" and not getattr(
                self, "_execution_guard", False
            ):
                return {
                    "status": "error",
                    "message": "Execution requires human confirmation. Use intent_router first.",
                }
            if tool_name == "add_todo":
                ...
                with self.console.status("[cyan]Executing add_todo...", spinner="dots"):
                    response = requests.post(
                        f"{self.base_url}/tasks", json={"name": f"[TODO] {content}"}
                    )
                response.raise_for_status()
                ...
                if not task_ids:
                    return {
                        "status": "error",
                        "message": "Task ID list cannot be empty.",
                    }
                results = []
                for task_id in task_ids:
                    with self.console.status(
                        f"[cyan]Completing todo {task_id}...", spinner="dots"
                    ):
                        response = requests.put(
                            f"{self.base_url}/tasks/{task_id}", json={"status": "done"}
                        )
                ...
                with self.console.status(
                    f"[cyan]Searching: {query}...", spinner="dots"
                ):
                    resp = requests.post(
                ...
                table = Table(
                    title=f"Web Search: {data.get('query', query)}",
                    show_lines=False,
                    expand=True,
                )
            ...
            self.console.print(
                f"[bold red]API call failed (HTTP Error). Status: {getattr(e.response, 'status_code', 'N/A')}. Details: {error_details}[/bold red]"
            )
            return {
                "status": "error",
                "message": f"API call failed during tool execution. Details: {error_details}",
            }
        except requests.RequestException as e:
            self.console.print(
                f"[bold red]API call failed (Network Error). Could not connect to {self.base_url}. Is the backend server running? Details: {e}[/bold red]"
            )
            return {
                "status": "error",
            ...
            self.console.print(
                f"[bold red]An unexpected error occurred during tool execution: {e}[/bold red]"
            )
        ...
        try:
            if action in {"show_plan", "show_tasks"}:
                ...
                if not title:
                    return {
                        "status": "error",
                        "message": "Missing title for show_* action",
                    }
                # Try fetch tasks directly
                encoded_title = urllib.parse.quote(title)
                resp = requests.get(
                    f"{self.base_url}/plans/{encoded_title}/tasks", timeout=20
                )
                if resp.status_code != 200:
                    # Fallback: propose + approve, then refetch
                    self._propose_plan({"goal": title})
                    requests.post(
                        f"{self.base_url}/plans/approve",
                        json={"title": title},
                        timeout=20,
                    )
                    resp = requests.get(
            ...
            if action == "execute_task":
                ...
                if not isinstance(task_id, int):
                    return {
                        "status": "error",
                        "message": "Missing/invalid task_id for execute_task",
                    }
                title = args.get("title") or ""
                output_filename = args.get("output_filename")
                # Confirm with user
                confirm = self._confirm(
                    f"Confirm execute task {task_id}{' → ' + output_filename if output_filename else ''}? [y/N] "
                )
                if not confirm:
    ...
    def _render_message(self, role: str, content: str) -> None:
        try:
            ...
            style = (
                "cyan"
                if role == "assistant"
                else ("magenta" if role == "tool" else "white")
            )
            if self._looks_like_markdown(content):
                try:
                    from rich.markdown import Markdown as _MD
                ...
                body = content
            self.console.print(
                Panel(body, title=title, border_style=style, expand=True)
            )
        except Exception:
        ...
        try:
            ...
            if tool_name == "web_search":
                ...
                if isinstance(results, list) and len(results) > 3:
                    ...
                    out["total_results"] = max(
                        out.get("total_results", len(results)), len(results)
                    )
    ...
    def _propose_plan(self, tool_args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not goal:
            return {
                "status": "error",
                "message": "Goal for proposing a plan cannot be empty.",
            }

        self.console.print(f"🤖 Proposing a plan for: '{goal}'...")
        self.console.print(
            "⏳ Contacting LLM to generate the plan... this may take several minutes. Timeout is set to 300 seconds."
        )
        propose_response = requests.post(
            f"{self.base_url}/plans/propose", json={"goal": goal}, timeout=300
        )
        ...
        if not plan_data or "title" not in plan_data or not plan_data.get("tasks"):
            try:
                ...
                if title:
                    encoded_title = urllib.parse.quote(title)
                    r = requests.get(
                        f"{self.base_url}/plans/{encoded_title}/tasks", timeout=10
                    )
        ...
        if not plan_data or "title" not in plan_data or not plan_data.get("tasks"):
            return {
                "status": "error",
                "message": "LLM failed to generate a valid plan with tasks.",
            }

        self.console.print(
            f"✅ Plan '{plan_data['title']}' proposed successfully. Waiting for user confirmation."
        )
    ...
    def _decompose_task(self, tool_args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.console.print(
            f"✅ Task {task_id} decomposed successfully into {len(result.get('subtasks', []))} sub-tasks."
        )
    ...
    def _visualize_plan(self, tool_args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            filepath = os.path.abspath(
                f"plan_{''.join(c for c in title if c.isalnum())}.html"
            )
            ...
            self.console.print(
                f"📊 Plan visualization opened in your browser. File saved at: {filepath}"
            )
            return {
                "status": "success",
                "message": f"Plan visualized and opened in browser at {filepath}",
            }
        except Exception as e:
            self.console.print(
                f"[bold red]Could not generate or open visualization: {e}[/bold red]"
            )
    ...
    def _execute_atomic_task(self, tool_args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not title or not isinstance(task_id, int):
            return {
                "status": "error",
                "message": "Invalid title or task_id for executing a single task.",
            }
        ...
        self.console.print(
            f"🚀 Executing atomic task ID {task_id} for plan '{title}'..."
        )

        try:
            tasks_response = requests.get(
                f"{self.base_url}/plans/{urllib.parse.quote(title)}/tasks"
            )
            ...
            approve_response = requests.post(
                f"{self.base_url}/plans/approve", json=plan_data, timeout=30
            )
            approve_response.raise_for_status()
            self.console.print(f"✅ Plan '{title}' approved/updated in database.")
        except Exception as e:
            self.console.print(
                f"[yellow]Warning: Could not re-approve plan before execution. This might fail if tasks are not yet in DB. Error: {e}[/yellow]"
            )
        ...
        run_response = requests.post(
            f"{self.base_url}/run", json=run_payload, timeout=1800
        )
        ...
        try:
            ...
            pass
        task_result = next(
            (r for r in run_result.get("results", []) if r.get("id") == task_id), None
        )

        if task_result and task_result.get("status") in ("done", "completed"):
            self.console.print(f"✅ Task {task_id} executed successfully.")
            output_response = requests.get(
                f"{self.base_url}/tasks/{task_id}/output", timeout=60
            )
            if output_response.status_code == 200:
                task_output = output_response.json().get(
                ...
                return {
                    "status": "success",
                    "task_id": task_id,
                    "result": "Task completed, but could not retrieve output.",
                }
        else:
            self.console.print(f"⚠️ Task {task_id} failed or status unknown.")
            return {
                "status": "error",
                "message": f"Execution of task {task_id} failed.",
                "details": task_result,
            }
