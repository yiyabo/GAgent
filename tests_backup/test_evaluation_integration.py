...
def test_complete_evaluation_system():
    ...
    task_id = default_repo.create_task(
        name="Write a comprehensive introduction to bacteriophage therapy", status="pending", task_type="atomic"
    )
    ...
    try:
        ...
        result = execute_task_with_evaluation(
            task=task, repo=default_repo, max_iterations=2, quality_threshold=0.7, use_context=False
        )
        ...
        for i, eval_record in enumerate(history, 1):
            print(
                f"  Iteration {i}: score={eval_record['overall_score']:.3f}, needs_revision={eval_record['needs_revision']}"
            )
        ...
        print(
            f"✓ Evaluation stats: {stats['total_evaluations']} total evaluations, avg score: {stats['average_score']:.3f}"
        )
        ...
        if len(history) >= 2:
            ...
            print(
                f"✓ Content improvement: {first_score:.3f} → {last_score:.3f} {'(improved!)' if improvement else '(no improvement)'}"
            )
