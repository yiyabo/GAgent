...
def test_api_endpoints():
    ...
    for task_data in test_plan["tasks"]:
        task_id = repo.create_task(name=task_data["name"], status="completed", priority=task_data["priority"])
    ...
    try:
        response = requests.post(
            f"http://127.0.0.1:8000/tasks/{task_ids[0]}/rerun", json={"use_context": False}, timeout=30
        )
    ...
    try:
        response = requests.post(
            ...
            json={"task_ids": task_ids[:2], "use_context": False},
            timeout=30,
        )
        if response.status_code == 200:
            result = response.json()
...
def test_cli_rerun():
    ...
    try:
        ...
        cmd = [sys.executable, "-m", "cli.main", "--rerun-interactive", "--title", "Test Rerun Plan", "--use-context"]
...
def test_task_listing():
    ...
    for i, status in enumerate(statuses):
        task_id = repo.create_task(name=f"Test Task {i+1}", status=status, priority=(i + 1) * 10)
        task_ids.append(task_id)
