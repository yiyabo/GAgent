...
from app.services.structure_prior import (
    StructurePriorCalculator,
    get_structure_prior_calculator,
)
...
def sample_tasks(setup_test_db):
    ...
    return {"parent": parent_id, "child1": child1_id, "child2": child2_id, "independent": independent_id, "repo": repo}
...
def test_dependency_weight_calculation(sample_tasks):
    ...
    # 测试requires依赖关系
    weights = calculator.compute_structure_weights(data["child1"], [data["child2"]])
...
def test_hierarchy_weight_calculation(sample_tasks):
    ...
    # 测试父子关系
    weights = calculator.compute_structure_weights(data["parent"], [data["child1"], data["child2"]])
...
def test_self_reference_weight(sample_tasks):
    ...
    # 测试自引用
    weights = calculator.compute_structure_weights(data["parent"], [data["parent"]])
...
def test_apply_structure_weights():
    ...
    semantic_scores = {1: 0.8, 2: 0.6, 3: 0.4}

    structure_weights = {1: 0.2, 2: 0.8, 3: 0.1}

    # 应用结构权重
    combined_scores = calculator.apply_structure_weights(semantic_scores, structure_weights, alpha=0.3)
...
def test_structure_explanation(sample_tasks):
    ...
    # 获取权重解释
    explanation = calculator.get_structure_explanation(data["parent"], data["child1"])
...
def test_nonexistent_task_handling(sample_tasks):
    ...
    # 使用不存在的任务ID
    weights = calculator.compute_structure_weights(999, [data["parent"]])
...
def test_cache_functionality(sample_tasks):
    ...
    # 第一次计算
    weights1 = calculator.compute_structure_weights(data["parent"], [data["child1"], data["child2"]])

    # 第二次计算（应该使用缓存）
    weights2 = calculator.compute_structure_weights(data["parent"], [data["child1"], data["child2"]])
    ...
    # 再次计算
    weights3 = calculator.compute_structure_weights(data["parent"], [data["child1"], data["child2"]])
...
def test_weight_bounds():
    ...
    combined_scores = calculator.apply_structure_weights(semantic_scores, structure_weights, alpha=0.5)
