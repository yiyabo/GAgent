...
class TestThreadSafetyEmbeddings:
    ...
    def test_concurrent_embedding_generation(self):
        ...
        # 测试数据
        test_texts_sets = [[f"concurrent_test_{i}_{j}" for j in range(5)] for i in range(10)]
    ...
    def test_high_concurrency_stress(self):
        ...
        ), f"Operations mismatch: {total_operations} vs {expected_operations}"

        # 性能统计
        ops_per_second = total_operations / total_time
        logger.info(
            f"Stress test completed: {total_operations} operations in {total_time:.2f}s ({ops_per_second:.2f} ops/sec)"
        )
