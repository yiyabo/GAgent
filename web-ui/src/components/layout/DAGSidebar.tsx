...
const formatRoundedPercent = (value: number): string => `${Math.round(value)}%`;
...
const DAGSidebar: React.FC = () => {
  ...
  const optimization = (meta.plan_optimization ?? meta.planOptimization ?? null) as Record<string, any> | null;
  const optimizeBefore = toNumber(
    optimization?.overall_score_before ?? optimization?.review_before?.overall_score ?? null
  );
  const optimizeAfter = toNumber(
    optimization?.overall_score_after ?? optimization?.review_after?.overall_score ?? null
  );
  const optimizeDelta = toNumber(
    optimization?.overall_score_delta
    ?? (optimizeBefore != null && optimizeAfter != null ? optimizeAfter - optimizeBefore : null)
  );
  const appliedChangeCount = toNumber(
    optimization?.applied_change_count ?? optimization?.appliedChangeCount ?? null
  );
  const dimensionDeltaSource = (optimization?.dimension_score_deltas ?? null) as Record<string, any> | null;
  const dimensionDeltas: Record<string, number> = {};
  if (dimensionDeltaSource && typeof dimensionDeltaSource === 'object') {
  for (const [k, v] of Object.entries(dimensionDeltaSource)) {
  const num = toNumber(v);
  if (num != null) dimensionDeltas[k] = num;
  }
  }
  return {
  origin,
  ...
  summary: typeof optimization?.summary === 'string' ? optimization.summary : null,
  dimensionDeltas,
  },
  };
  }, [planTree]);
  ...
  const optimization = planEvaluation.optimization;
  const hasDims = Object.keys(dimScores).length > 0;
  const hasOptimizeScores = optimization.before != null || optimization.after != null;
  ...
  const optimizationTooltipContent = Object.keys(optimization.dimensionDeltas).length > 0 ? (
  <div style={{ fontSize: 12, lineHeight: 1.6 }}>
  {Object.entries(optimization.dimensionDeltas)
  .sort(([a], [b]) => a.localeCompare(b))
  .map(([k, v]) => (
  <div key={k} style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
  <span style={{ opacity: 0.85 }}>{k}</span>
  <span style={{ fontWeight: 600, color: v >= 0 ? '#389e0d' : '#cf1322' }}>
  {v >= 0 ? '+' : ''}{Math.round(v)}
  </span>
  </div>
  ))}
  ...
  <div style={{ fontSize: 12, opacity: 0.85 }}>
  {optimization.summary || 'No optimize delta breakdown available yet.'}
  </div>
  );
  ...
  >
  <Space size={8} wrap>
  {optimization.before != null && optimization.after != null && (
  <Tooltip title={optimizationTooltipContent} placement="bottomLeft">
  <Tag color="gold" style={{ marginInlineEnd: 0 }}>
  Optimize {formatRoundedPercent(optimization.before)} {'->'} {formatRoundedPercent(optimization.after)}
  </Tag>
  </Tooltip>
  )}
  {optimization.delta != null && (
  <Tag color={optimization.delta >= 0 ? 'green' : 'red'} style={{ marginInlineEnd: 0 }}>
  {formatScoreDelta(optimization.delta)}
  </Tag>
  )}
  {optimization.appliedChangeCount != null && (
  <Tag color="default" style={{ marginInlineEnd: 0 }}>
  {Math.round(optimization.appliedChangeCount)} changes
  </Tag>
  )}
  {optimization.autoGenerated && (
  ...
  {new Date(optimization.optimizedAt).toLocaleString()}
  </Text>
  )}
  </div>
  )}
