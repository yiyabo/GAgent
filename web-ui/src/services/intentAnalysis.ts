...
async function executeTaskCreate(
  ...
  try {
    ...
    const response = await fetch(`${envConfig.apiBaseUrl}/tasks/intelligent-create`, {
      method: 'POST',
      headers: {
...
async function executeTaskDecompose(
  ...
  try {
    ...
    const tasksResponse = await fetch(`${envConfig.apiBaseUrl}/tasks?session_id=${sessionId}`);
...
async function performRealTaskDecomposition(rootTask: any, userRequest: string, sessionId: string): Promise<{
  ...
  try {
    ...
    // 调用后端任务分解API
    const decompositionResponse = await fetch(`${envConfig.apiBaseUrl}/tasks/${rootTask.id}/decompose`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
