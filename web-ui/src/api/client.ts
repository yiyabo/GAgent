...
const createApiClient = (): AxiosInstance => {
  const client = axios.create({
    baseURL: envConfig.apiBaseUrl,  // 直接连接后端API
    timeout: 120000,
    headers: {
...
export const checkApiHealth = async (): Promise<{
  ...
  try {
    ...
    const healthResponse = await fetch(`${envConfig.apiBaseUrl}/health`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (healthResponse.ok) {
      ...
      const llmResponse = await fetch(`${envConfig.apiBaseUrl}/health/llm?ping=true`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
