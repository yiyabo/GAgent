...
df = pd.read_csv(uploads() / "experiment.csv")
stats = df.groupby("组别")["测定值"].agg(["mean", "std"])
stds = df.groupby("组别")["测定值"].std(ddof=0)

text = report_text("experiment_report.md", 1500)
require_sections(text, ["## 数据概览", "## 分组统计", "## 组间对比", "## 结论"])
assert has_md_table(text), "no markdown table in report"
for grp in stats.index:
    assert grp in text, f"{grp} missing"
    assert num_in_text(text, float(stats.loc[grp, "mean"])), f"{grp} mean missing"
    ok_std = (num_in_text(text, float(stats.loc[grp, "std"]))
              or num_in_text(text, float(stds.loc[grp])))
    assert ok_std, f"{grp} std missing"
