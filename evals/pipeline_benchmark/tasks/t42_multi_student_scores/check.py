...
df = pd.read_csv(uploads() / "scores2.csv")
truth = df.groupby("班级")[["数学", "英语", "物理"]].mean()
out = find_file("class_avg.csv")
assert out, "results/class_avg.csv not found"
got = pd.read_csv(out).set_index("班级")
for cls in truth.index:
    for sub, col in (("数学", "数学均分"), ("英语", "英语均分"), ("物理", "物理均分")):
        assert close2(got.loc[cls, col], truth.loc[cls, sub]), f"{cls} {sub} wrong"
check_png("score_dist.png")
best_cls = truth["数学"].idxmax()
text = report_text("score_report.md", 600)
require_sections(text, ["## 数据概览", "## 班级对比", "## 结论"])
assert best_cls in text and num_in_text(text, float(truth["数学"].max())), "best class missing"
