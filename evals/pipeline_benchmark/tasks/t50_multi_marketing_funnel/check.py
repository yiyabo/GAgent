...
df = pd.read_csv(uploads() / "campaigns.csv")
totals = {c: int(df[c].sum()) for c in ("曝光", "点击", "注册", "付费")}
out = find_file("funnel.csv")
assert out, "results/funnel.csv not found"
got = pd.read_csv(out)
assert list(got["环节"]) == ["曝光", "点击", "注册", "付费"], "stage order wrong"
for _, row in got.iterrows():
    assert int(row["总量"]) == totals[row["环节"]], f"{row['环节']} total wrong"
check_png("funnel_bar.png")
ctr = totals["点击"] / totals["曝光"] * 100
text = report_text("marketing_report.md", 600)
require_sections(text, ["## 数据概览", "## 漏斗分析", "## 结论"])
assert num_in_text(text, totals["曝光"], 0), "total impressions missing"
assert num_in_text(text, totals["付费"], 0), "total payments missing"
assert num_in_text(text, ctr), f"CTR {ctr:.2f}% missing"
