...
df = pd.read_excel(uploads() / "sales.xlsx", sheet_name="销售记录")
total = float((df["销量"] * df["单价"]).sum())
top_prod = str(df.groupby("产品")["销量"].sum().sort_values(ascending=False).index[0])

assert num_in_text(ANSWER, len(df), 0), f"row count {len(df)} missing"
assert num_in_text(ANSWER, total), f"total {total:.2f} missing"
assert top_prod in ANSWER, f"top product {top_prod} missing"
