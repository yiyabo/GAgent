...
df = pd.read_csv(uploads() / "prod_sales.csv")
truth = df.groupby("类别")["销售额"].sum().sort_values(ascending=False)
out = find_file("cat_summary.csv")
assert out, "results/cat_summary.csv not found"
got = pd.read_csv(out)
assert list(got["类别"]) == list(truth.index), "order wrong"
for a, b in zip(got["销售额合计"], truth.values):
    assert close2(a, b), "sum wrong"
check_png("cat_bar.png")
text = report_text("category_report.md", 600)
require_sections(text, ["## 数据概览", "## 类别分析", "## 结论"])
assert str(truth.index[0]) in text and num_in_text(text, float(truth.iloc[0])), "top category missing"
