...
df = pd.read_csv(uploads() / "daily.csv")
truth = df.groupby(df["日期"].str[:7])["销售额"].sum()
out = find_file("monthly_sales.csv")
assert out, "results/monthly_sales.csv not found"
got = pd.read_csv(out)
assert len(got) == 12 and list(got["月份"]) == list(truth.index), "months wrong"
for a, b in zip(got["月销售额"], truth.values):
    assert close2(a, b), "monthly sum wrong"
check_png("sales_trend.png")
text = report_text("sales_report.md", 600)
require_sections(text, ["## 数据概览", "## 趋势分析", "## 结论"])
assert num_in_text(text, float(df["销售额"].sum())), "annual total missing in report"
