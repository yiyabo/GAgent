...
df = pd.read_csv(uploads() / "survey.csv")
overall = float(df["评分"].mean())
by_q = df.groupby("题目")["评分"].mean().sort_values()
worst_q, worst_v = str(by_q.index[0]), float(by_q.iloc[0])

text = report_text("survey_report.md", 1500)
require_sections(text, ["## 数据概览", "## 各题得分", "## 低分问题", "## 改进建议"])
assert has_md_table(text), "no markdown table in report"
assert num_in_text(text, overall), f"overall mean {overall:.2f} missing"
assert worst_q in text and num_in_text(text, worst_v), f"worst question {worst_q} missing"
