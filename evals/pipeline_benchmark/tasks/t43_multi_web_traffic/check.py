...
df = pd.read_csv(uploads() / "web_traffic.csv")
ts = pd.to_datetime(df["日期"], format="%Y-%m-%d")
weeks = (ts - pd.to_timedelta(ts.dt.weekday, unit="D")).dt.strftime("%Y-%m-%d")
truth = df.groupby(weeks)["PV"].sum()
out = find_file("weekly_pv.csv")
assert out, "results/weekly_pv.csv not found"
got = pd.read_csv(out)
assert list(got["周"]) == list(truth.index), "weeks wrong"
for a, b in zip(got["PV合计"], truth.values):
    assert int(a) == int(b), "weekly pv wrong"
check_png("pv_trend.png")
text = report_text("traffic_report.md", 600)
require_sections(text, ["## 数据概览", "## 趋势分析", "## 结论"])
assert num_in_text(text, int(df["PV"].sum()), 0), "total PV missing in report"
