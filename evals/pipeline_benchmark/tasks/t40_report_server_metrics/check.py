...
df = pd.read_csv(uploads() / "server_metrics.csv")
cpu_mean, cpu_max = float(df["CPU利用率"].mean()), float(df["CPU利用率"].max())
mem_mean = float(df["内存利用率"].mean())

text = report_text("server_report.md", 1500)
require_sections(text, ["## 数据概览", "## CPU 分析", "## 内存分析", "## 结论与建议"])
assert has_md_table(text), "no markdown table in report"
assert num_in_text(text, cpu_mean), f"cpu mean {cpu_mean:.2f} missing"
assert num_in_text(text, cpu_max), f"cpu max {cpu_max:.2f} missing"
assert num_in_text(text, mem_mean), f"mem mean {mem_mean:.2f} missing"
