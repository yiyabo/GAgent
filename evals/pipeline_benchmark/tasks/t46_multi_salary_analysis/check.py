...
df = pd.read_csv(uploads() / "employees.csv")
truth = df.groupby("部门")["月薪"].agg(["mean", "median"])
out = find_file("dept_salary.csv")
assert out, "results/dept_salary.csv not found"
got = pd.read_csv(out).set_index("部门")
for d in truth.index:
    assert close2(got.loc[d, "平均月薪"], truth.loc[d, "mean"]), f"{d} mean wrong"
    assert close2(got.loc[d, "中位数月薪"], truth.loc[d, "median"]), f"{d} median wrong"
check_png("salary_box.png")
text = report_text("salary_report.md", 600)
require_sections(text, ["## 数据概览", "## 部门对比", "## 结论"])
assert num_in_text(text, float(df["月薪"].mean())), "company avg missing in report"
