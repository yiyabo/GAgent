...
text = (uploads() / "sync_design.md").read_text(encoding="utf-8")
h2 = [ln for ln in text.splitlines() if ln.startswith("## ")]
assert num_in_text(ANSWER, len(h2), 0), f"h2 count {len(h2)} missing"
assert num_in_text(ANSWER, 3842, 0), "QPS 3842 missing"
for mod in ("ingest", "transform", "sink"):
    assert mod in ANSWER, f"module {mod} missing"
