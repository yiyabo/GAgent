...
cfg = json.loads((uploads() / "config.json").read_text(encoding="utf-8"))
assert num_in_text(ANSWER, cfg["experiment"]["batch_size"], 0), "batch_size missing"
assert num_in_text(ANSWER, len(cfg["datasets"]), 0), "datasets length missing"
assert cfg["logging"]["level"] in ANSWER, "log level missing"
