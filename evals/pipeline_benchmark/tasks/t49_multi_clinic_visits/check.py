...
df = pd.read_csv(uploads() / "clinic.csv")
truth = df.groupby("科室")["就诊人次"].sum().sort_values(ascending=False)
out = find_file("dept_totals.csv")
assert out, "results/dept_totals.csv not found"
got = pd.read_csv(out)
assert list(got["科室"]) == list(truth.index), "order wrong"
for a, b in zip(got["总人次"], truth.values):
    assert int(a) == int(b), "dept total wrong"
check_png("clinic_heatmap.png")
text = report_text("clinic_report.md", 600)
require_sections(text, ["## 数据概览", "## 科室分析", "## 结论"])
assert num_in_text(text, int(df["就诊人次"].sum()), 0), "annual total missing"
assert str(truth.index[0]) in text, "top dept missing"
