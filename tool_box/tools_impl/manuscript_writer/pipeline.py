"""Manuscript writer pipeline: the ``manuscript_writer_handler`` orchestration.

The 1388-line handler and its two module-level helpers live here; everything
else is imported from the sibling modules of this package.

Patch surfaces: the handler calls ``_chat`` and ``_build_llm_service`` through
the late-bound ``_facade()`` accessor (14 test sites each patch them on the
package namespace) and reads ``_PROJECT_ROOT`` the same way. The success/failure
payload keys and the ``.manuscript_writer_<ts>`` workspace name are unchanged.
"""
...
async def manuscript_writer_handler(
    ...
    try:
        ...
        def _to_rel(path: Optional[Path]) -> Optional[str]:
            if path is None:
                return None
            return str(path.relative_to(_facade()._PROJECT_ROOT))
        ...
        async def _gen_eval_section_core(
            ...
            for attempt in range(1, max_revisions + 1):
                ...
                eval_raw = await _facade()._chat(eval_llm, eval_prompt, eval_model, max_tokens=_MAX_TOKENS_EVAL, purpose="manuscript_writer:eval")
                ...
                text = await _facade()._chat(gen_llm, revision_prompt, gen_model, max_tokens=_MAX_TOKENS_SECTION, purpose="manuscript_writer:section")
        ...
        gen_llm, gen_model = _facade()._build_llm_service(generation_provider, gen_model)
        eval_llm, eval_model = _facade()._build_llm_service(evaluation_provider, eval_model)
        merge_llm, merge_model_name = _facade()._build_llm_service(merge_provider, merge_model_name)
        final_polish_eval_llm, _ = _facade()._build_llm_service(
            evaluation_provider,
            eval_model,
            timeout=final_polish_llm_timeout_sec,
        )
        final_polish_merge_llm, _ = _facade()._build_llm_service(
            merge_provider,
            merge_model_name,
            timeout=final_polish_llm_timeout_sec,
        )
        ...
        analysis_memo = await _facade()._chat(gen_llm, analysis_prompt, gen_model, max_tokens=_MAX_TOKENS_MEMO, purpose="manuscript_writer:memo")
        analysis_file.write_text(analysis_memo, encoding="utf-8")
        logger.info(
        ...
        async def _smooth_transition(sec_a: str, sec_b: str) -> Optional[str]:
            ...
            return await _facade()._chat(merge_llm, prompt, merge_model_name, max_tokens=_MAX_TOKENS_TRANSITION, purpose="manuscript_writer:merge")
            ...
            try:
                for attempt in range(1, final_polish_max_revisions + 1):
                    ...
                    polished_candidate = await _maybe_wait_with_timeout(
                        _facade()._chat(final_polish_merge_llm, polish_prompt, merge_model_name, max_tokens=_MAX_TOKENS_MERGE, purpose="manuscript_writer:polish"),
                    ...
                    review_raw = await _maybe_wait_with_timeout(
                        _facade()._chat(final_polish_eval_llm, review_prompt, eval_model, max_tokens=_MAX_TOKENS_EVAL, purpose="manuscript_writer:eval"),
                        final_polish_step_timeout_sec,
                    )
