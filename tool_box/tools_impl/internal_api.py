    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            if method.upper() == "POST":
                response = await client.post(full_url, json=data, timeout=timeout)
            elif method.upper() == "GET":
                response = await client.get(full_url, params=data, timeout=timeout)
            elif method.upper() == "PUT":
                response = await client.put(full_url, json=data, timeout=timeout)
            elif method.upper() == "DELETE":
                response = await client.delete(full_url, timeout=timeout)
            else:
                return {
                    "success": False,
            ...
            if not result["success"]:
                ...
                logger.error(
                    f"❌ Internal API call failed: {method} {full_url} - {response.status_code}"
                )
            else:
...
internal_api_tool = {
    ...
    "parameters_schema": {
        ...
        "properties": {
            ...
            "data": {"type": "object", "description": "请求数据 (用于POST/PUT请求)"},
            "timeout": {
                "type": "number",
