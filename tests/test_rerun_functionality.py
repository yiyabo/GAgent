...
def test_api_endpoints():
    ...
    for task_data in test_plan["tasks"]:
        task_id = repo.create_task(
            name=task_data["name"], status="completed", priority=task_data["priority"]
        )
...
def test_task_listing():
    ...
    for i, status in enumerate(statuses):
        task_id = repo.create_task(
            name=f"Test Task {i + 1}", status=status, priority=(i + 1) * 10
        )
        task_ids.append(task_id)
...
def run_comprehensive_test():
    ...
    if not server_running:
        ...
        print(
            "Please start the server with: conda run -n LLM python -m uvicorn app.main:app --reload"
        )
        return False
    ...
    try:
        ...
        print(
            f"  python -m cli.main --rerun-selected --task-ids {','.join(map(str, task_ids[:2]))}"
        )
