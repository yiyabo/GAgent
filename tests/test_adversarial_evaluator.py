...
def test_adversarial_evaluator():
    ...
    try:
        ...
        # Success criteria
        success = (
            result['rounds_completed'] >= 1 and
            result['best_robustness_score'] > 0.3 and  # Some improvement
            result['metadata']['total_criticisms'] > 0  # Critic found issues
        )
        
        assert success, f"Adversarial evaluation failed: rounds={result['rounds_completed']}, score={result['best_robustness_score']:.3f}, criticisms={result['metadata']['total_criticisms']}"
