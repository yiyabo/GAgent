...
def sample_tasks(setup_test_db):
    ...
    child1_id = repo.create_task(
        "子任务1", status="pending", priority=2, parent_id=parent_id
    )
    child2_id = repo.create_task(
        "子任务2", status="pending", priority=3, parent_id=parent_id
    )
    ...
    repo.create_link(
        independent_id, parent_id, "refers"
    )  # parent refers to independent
