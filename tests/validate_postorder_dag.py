...
class TaskDAGValidator:
    ...
    def create_sample_project(self) -> Dict[str, int]:
        ...
        # 创建项目根任务
        project_id = self.repo.create_task(
            "噬菌体抗药性研究项目", 
            priority=10, 
            task_type="composite"
        )
        self.repo.upsert_task_input(
            project_id, 
            "整合所有研究模块，完成噬菌体对抗细菌耐药性机制的综合研究报告"
        )
        
        # 文献调研模块
        literature_id = self.repo.create_task(
            "文献调研模块", 
            priority=20, 
        ...
        phage_review_id = self.repo.create_task(
            "噬菌体生物学文献综述", 
            priority=21, 
            parent_id=literature_id,
            task_type="atomic"
        )
        self.repo.upsert_task_input(
            phage_review_id, 
            "系统性回顾噬菌体的生物学特性、分类、生命周期和宿主特异性相关文献"
        )
        ...
        self.repo.upsert_task_input(
            resistance_review_id, 
            "分析细菌耐药性产生机制、传播途径和当前治疗挑战的相关研究"
        )
        
        # 实验设计模块
        experiment_id = self.repo.create_task(
            "实验设计模块", 
            priority=30, 
            parent_id=project_id,
            task_type="composite"
        ...
        self.repo.upsert_task_input(
            protocol_design_id, 
            "设计噬菌体-细菌相互作用实验方案，包括感染效率、裂解动力学等测试"
        )
        
        # 数据分析模块
        analysis_id = self.repo.create_task(
            "数据分析模块", 
            priority=40, 
            parent_id=project_id,
            task_type="composite"
        ...
        self.repo.upsert_task_input(
            ...
            "对实验数据进行统计学检验，评估噬菌体治疗效果的显著性"
        )
        
        # 成果输出模块
        output_id = self.repo.create_task(
            "成果输出模块", 
            priority=50, 
            parent_id=project_id,
            task_type="composite"
        ...
        self.repo.upsert_task_input(
            output_id, 
            "整合研究成果，完成学术论文撰写和研究报告"
        )
        
        # 成果输出子任务
        paper_writing_id = self.repo.create_task(
            "学术论文撰写", 
            priority=51, 
            parent_id=output_id,
            task_type="atomic"
        ...
        self.repo.upsert_task_input(
            paper_writing_id, 
            "撰写高质量学术论文，阐述噬菌体抗药性研究的发现和意义"
        )
        
        presentation_id = self.repo.create_task(
            "学术会议报告", 
            priority=52, 
            parent_id=output_id,
            task_type="atomic"
