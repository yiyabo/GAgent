...
class TestRerunCoreLogic:
    ...
    def test_single_task_rerun_logic(self, task_repo, mock_execute_task):
        """测试单个任务重新执行的核心逻辑"""
        # 创建测试任务
        task_id = task_repo.create_task("测试任务", status="completed", task_type="atomic")
    ...
    def test_subtree_rerun_logic(self, task_repo, mock_execute_task):
        """测试子树重新执行的核心逻辑"""
        # 创建父任务
        parent_id = task_repo.create_task("父任务", status="completed", task_type="composite")
        
        # 创建子任务
        child1_id = task_repo.create_task("子任务1", parent_id=parent_id, status="completed", task_type="atomic")
        child2_id = task_repo.create_task("子任务2", parent_id=parent_id, status="completed", task_type="atomic")
    ...
    def test_rerun_empty_subtree(self, task_repo, mock_execute_task):
        """测试重新执行没有子任务的子树"""
        task_id = task_repo.create_task("独立任务", status="completed", task_type="atomic")
    ...
    def test_rerun_preserves_priority_order(self, task_repo, mock_execute_task):
        ...
        # 创建不同优先级的子任务
        high_priority_id = task_repo.create_task("高优先级任务", parent_id=parent_id, priority=10, status="completed", task_type="atomic")
        low_priority_id = task_repo.create_task("低优先级任务", parent_id=parent_id, priority=100, status="completed", task_type="atomic")
    ...
    def test_rerun_with_dependencies(self, task_repo, mock_execute_task):
        """测试重新执行有依赖关系的任务"""
        # 创建任务和依赖关系
        task1_id = task_repo.create_task("前置任务", status="completed", task_type="atomic")
        task2_id = task_repo.create_task("依赖任务", status="completed", task_type="atomic")
...
class TestRerunDataIntegrity:
    ...
    def test_status_transitions(self, task_repo, mock_execute_task):
        """测试状态转换的正确性"""
        task_id = task_repo.create_task("状态测试任务", status="completed", task_type="atomic")
