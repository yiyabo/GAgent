...
def test_repository_links_and_dependencies(tmp_path, monkeypatch):
    ...
    assert any(
        x["from_id"] == a and x["to_id"] == b and x["kind"] == "requires"
        for x in inbound
    )
    assert any(
        x["from_id"] == c and x["to_id"] == b and x["kind"] == "refers" for x in inbound
    )
...
def test_gather_context_minimal(tmp_path, monkeypatch):
    ...
    first_dep = next(
        (s for s in secs if str(s.get("kind", "")).startswith("dep:")), None
    )
    assert first_dep is not None, "No dependency section found"
...
def test_api_context_endpoints(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert r.status_code == 200
        tasks = r.json()
        one: Dict[str, Any] = next((t for t in tasks if "One" in t["name"]), None)
        two: Dict[str, Any] = next((t for t in tasks if "Two" in t["name"]), None)
        assert one is not None, "Task 'One' not found"
        assert two is not None, "Task 'Two' not found"

        # Create a requires link: One -> Two
        r = client.post(
            "/context/links",
            json={"from_id": one["id"], "to_id": two["id"], "kind": "requires"},
        )
        ...
        data = r.json()
        assert any(
            link["from_id"] == one["id"] and link["kind"] == "requires"
            for link in data.get("inbound", [])
        )
        ...
        r = client.post("/run", json={"plan_id": plan_id, "use_context": True})
        assert r.status_code == 200
...
def test_gather_context_with_semantic_retrieval(tmp_path, monkeypatch):
    ...
    embedding = embeddings_service.get_single_embedding(
        "banana banana something relevant"
    )
    ...
    print(
        f"Task details: {[(t.get('id'), t.get('name'), bool(t.get('embedding_vector'))) for t in tasks_with_embeddings]}"
    )
    ...
    assert len(secs) >= 1, (
        f"Expected at least index section, got: {[s.get('kind') for s in secs]}"
    )
    assert secs[0].get("kind") == "index", (
        f"Expected first section to be index, got: {secs[0].get('kind')}"
    )

    # If we have search results, we should have retrieved sections
    if search_results:
        retrieved_sections = [s for s in secs if s.get("kind") == "retrieved"]
        assert len(retrieved_sections) > 0, (
            f"Expected retrieved sections when search found {len(search_results)} results"
        )
...
def test_api_context_preview_with_semantic_option(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert r.status_code == 200
        tasks = r.json()
        a = next((t for t in tasks if "A" in t["name"]), None)
        b = next((t for t in tasks if "B" in t["name"]), None)
        ...
        embeddings_service = get_embeddings_service()
        embedding = embeddings_service.get_single_embedding(
            "banana banana text for retrieval"
        )
        if embedding:
            embedding_json = embeddings_service.embedding_to_json(embedding)
