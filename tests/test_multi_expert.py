    ...
    try:
        ...
        if disagreements:
            ...
            for disagreement in disagreements:
                print(f"   {disagreement['field']}: {disagreement['lowest_scorer']}({disagreement['lowest_score']:.2f}) vs {disagreement['highest_scorer']}({disagreement['highest_score']:.2f})")
        else:
            print(f"\n✅ 专家意见一致，无重大分歧")
        ...
        # Success criteria
        success = (
            len(expert_evals) >= 2 and  # At least 2 experts evaluated
            consensus.get('overall_score', 0) > 0.1 and  # Some meaningful score
            consensus.get('consensus_confidence', 0) > 0.3  # Reasonable confidence
        )
        
        assert success, f"Multi-expert evaluation failed: experts={len(expert_evals)}, consensus_score={consensus.get('overall_score', 0):.3f}, confidence={consensus.get('consensus_confidence', 0):.3f}"
