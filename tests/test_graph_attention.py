...
from app.services.graph_attention import GraphAttentionReranker, get_graph_attention_reranker
...
def test_rerank_with_attention(sample_graph_data):
    ...
    # 执行重排
    reranked = reranker.rerank_with_attention(
        data['root'], candidates, data['embeddings'], alpha=0.3
    )
...
def test_attention_explanation(sample_graph_data):
    ...
    explanation = reranker.get_attention_explanation(
        data['root'], data['branch1'], data['embeddings']
    )
...
def test_single_candidate_handling(sample_graph_data):
    ...
    result = reranker.rerank_with_attention(
        data['root'], candidates, data['embeddings'], alpha=0.3
    )
...
def test_attention_weight_bounds(sample_graph_data):
    ...
    reranked = reranker.rerank_with_attention(
        data['root'], candidates, data['embeddings'], alpha=0.5
    )
...
def test_cache_functionality(sample_graph_data):
    ...
    # 第一次计算
    result1 = reranker.rerank_with_attention(
        data['root'], candidates, data['embeddings'], alpha=0.3
    )
    ...
    # 再次计算
    result2 = reranker.rerank_with_attention(
        data['root'], candidates, data['embeddings'], alpha=0.3
    )
