...
| Chat agent core | `routers/chat/agent.py` + 10 siblings | Facade (≤4000 lines) keeps `StructuredChatAgent`, the patchable module bindings, and both compat bridges verbatim; `unified_stream.py` holds the extracted `process_unified_stream` phases (SSE order locked by `app/tests/chat/test_unified_stream_event_golden.py`). `handle()` and `get_structured_response()` share `_run_response_guard_pipeline`. See `routers/chat/AGENTS.md`. |
