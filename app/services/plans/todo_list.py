...
_PHASE_LABEL_RULES: List[Tuple[str, List[str]]] = [
    ("Data Preparation", [
        ...
        r"数据.*(?:准备|收集|采集|下载|获取|导入)",
        r"(?:下载|获取|收集|采集)",
    ]),
    ("Literature Search", [
        r"文献.*(?:检索|搜索|收集|调研)",
        r"(?:检索|搜索).*(?:文献|论文|pubmed|pmc)",
        r"literature.*(?:search|retriev|collect|survey)",
        r"evidence.*collect",
        r"prisma",
        r"参考文献",
        r"references\.bib",
    ...
    ("Evidence Extraction", [
        r"提取.*(?:证据|文献|关键|核心)",
        r"(?:证据|文献).*提取",
        r"extract.*(?:evidence|finding|key)",
        r"evidence.*extract",
    ]),
    ("Writing", [
        r"撰写.*(?:章|初稿|论文|综述|引言|结论|摘要)",
        r"(?:章|初稿|论文|综述|引言|结论|摘要).*撰写",
        r"write.*(?:chapter|draft|paper|review|introduction|conclusion|abstract)",
        r"(?:chapter|draft|paper|review|introduction|conclusion|abstract).*write",
        r"第\d+章",
    ]),
    ("Assembly", [
    ...
    ("Review & Polish", [
        r"润色", r"校验", r"校对", r"审核", r"适配",
        r"引用.*(?:格式|校验|统一)",
        r"术语.*(?:统一|规范)",
        r"(?:格式|语言).*润色",
        r"期刊.*适配",
        r"citation.*valid", r"reference.*check",
        r"polish", r"proofread", r"format.*check",
    ]),
    ("Reporting", [
