            ...
            with self._lock:
                ...
                with sqlite3.connect(self.cache_db_path) as conn:
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (cache_key, content_hash, context_hash, evaluation_method, 
    ...
    def _add_to_memory_cache(self, cache_key: str, cached_data: Dict[str, Any]):
        ...
        if len(self.memory_cache) >= self.max_cache_size:
            ...
            oldest_keys = sorted(
                ...
                key=lambda k: self.memory_cache[k].get("created_at", "")
            )[:entries_to_remove]
        ...
        with self._lock:
            ...
            try:
                with sqlite3.connect(self.cache_db_path) as conn:
                    if evaluation_method:
                        cursor = conn.execute(
                            "SELECT COUNT(*) FROM evaluation_cache WHERE evaluation_method = ?",
                            (evaluation_method,)
                        )
                        persistent_cleared = cursor.fetchone()[0]
                        
                        conn.execute(
                            "DELETE FROM evaluation_cache WHERE evaluation_method = ?",
                            (evaluation_method,)
                        )
                    else:
                        cursor = conn.execute("SELECT COUNT(*) FROM evaluation_cache")
