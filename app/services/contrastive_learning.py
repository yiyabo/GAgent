...
class StructureContrastiveLearningDataGenerator:
    ...
    def generate_training_data(self, task_ids: Optional[List[int]] = None, 
                             max_samples: int = 1000) -> List[ContrastiveSample]:
        """
        生成结构对比学习训练数据
        ...
        for anchor_id in task_ids:
            ...
            # 为每个anchor任务生成多个样本
            task_samples = self._generate_samples_for_task(
                anchor_id, task_graph, task_texts
            )
    ...
    def _generate_samples_for_task(self, anchor_id: int, task_graph: Dict,
                                 task_texts: Dict[int, str]) -> List[ContrastiveSample]:
        """为单个任务生成对比学习样本"""
        samples = []
    ...
    def _compute_relation_scores(self, anchor_id: int, task_graph: Dict) -> Dict[int, float]:
        ...
        for task_id, task_info in task_graph.items():
            ...
            # 兄弟关系
            if (anchor_info.get('parent_id') and 
                anchor_info.get('parent_id') == task_info.get('parent_id')):
    ...
    def _determine_relation_type(self, anchor_id: int, positive_id: int, task_graph: Dict) -> str:
        ...
        if anchor_id == positive_info.get('parent_id'):
            return 'child'
        if (anchor_info.get('parent_id') and 
            anchor_info.get('parent_id') == positive_info.get('parent_id')):
            return 'sibling'
        
        return 'neighbor'
    
    def export_training_data(self, samples: List[ContrastiveSample], 
    ...
    def _export_csv(self, samples: List[ContrastiveSample], output_path: str) -> None:
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            ...
            writer.writerow([
                ...
                'negative_id', 'negative_text', 'relation_type', 'similarity_score'
            ])
            
            # 写入数据
            for sample in samples:
                writer.writerow([
                    sample.anchor_id, sample.anchor_text,
                    sample.positive_id, sample.positive_text,
                    sample.negative_id, sample.negative_text,
                    sample.relation_type, sample.similarity_score
                ])
