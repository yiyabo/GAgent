...
class UnifiedCache:
    ...
    def _add_to_l1(self, key: str, entry: CacheEntry):
        ...
        if len(self.l1_cache) >= self.l1_size:
            ...
            coldest_key = min(
                self.l1_cache.keys(),
                key=lambda k: self.l1_cache[k].get_heat_score()
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            l1_heat = sum(e.get_heat_score() for e in self.l1_cache.values()) / len(self.l1_cache) if self.l1_cache else 0
            l2_heat = sum(e.get_heat_score() for e in self.l2_cache.values()) / len(self.l2_cache) if self.l2_cache else 0
