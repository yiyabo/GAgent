...
class ThreadSafeAsyncManager:
    ...
    def __init__(self, batch_processor):
        ...
        self._async_executor = ThreadPoolExecutor(
            max_workers=5, 
            thread_name_prefix="embedding-async-safe"
        )
    ...
    def get_embeddings_async(self, texts: List[str], 
        ...
        future = self._async_executor.submit(
            self._async_embedding_worker, 
            texts, 
            callback,
            task_type="get_embeddings"
        )
    ...
    def get_single_embedding_async(self, text: str, 
                                 callback: Optional[Callable] = None) -> Future:
        """
        线程安全异步获取单个嵌入向量
        ...
        # 创建任务
        future = self._async_executor.submit(
            self._async_single_embedding_worker, 
            text, 
            callback,
            task_type="get_single_embedding"
        )
    ...
    def precompute_embeddings_async(self, texts: List[str], 
        ...
        # 创建任务
        future = self._async_executor.submit(
            self._async_precompute_worker, 
            texts, 
            progress_callback,
            task_type="precompute_embeddings"
        )
    ...
    def _async_embedding_worker(self, texts: List[str], 
    ...
    def _async_single_embedding_worker(self, text: str, 
                                     callback: Optional[Callable] = None,
    ...
    def _async_precompute_worker(self, texts: List[str], 
    ...
    def _cleanup_completed_tasks(self):
        ...
        with self._tasks_lock:
            ...
            # 清理元数据
            removed_metadata = [
                task_id for task_id in self._task_metadata.keys()
            ...
            # 清理回调
            removed_callbacks = [
                task_id for task_id in self._task_callbacks.keys()
                if task_id not in active_task_ids
            ]
    ...
    def get_async_status(self) -> Dict[str, Any]:
        ...
        with self._tasks_lock:
            ...
            completed_tasks = sum(1 for task in self._background_tasks 
                                if task.done() and not task.cancelled())
            failed_tasks = sum(1 for task in self._background_tasks 
        ...
        with self._stats_lock:
            avg_completion_time = (
                ...
                if self._completion_times else 0
            )
