...
class ThreadSafeEmbeddingCache:
    ...
    def __init__(self, cache_size: int = 10000, enable_persistent: bool = True):
        ...
        self._cache_lock = threading.RWLock()  # 需要实现读写锁
        ...
        logger.info(f"Thread-safe embedding cache initialized: memory_size={cache_size}, persistent={enable_persistent}")
        ...
        try:
            with self._get_db_connection() as conn:
                ...
                if row:
                    ...
                    # 加载到内存缓存
                    self._load_to_memory_cache(ThreadSafeCacheEntry(
                        text_hash=text_hash,
                        embedding=embedding,
                        model=model,
