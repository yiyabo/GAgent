...
class MetaEvaluator(LLMBasedEvaluator):
    ...
    def meta_evaluate(
        self, evaluation_history: List[Dict[str, Any]], content: str, task_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            meta_scores = self._calculate_meta_scores(consistency_analysis, bias_analysis, llm_meta_evaluation)

            # Generate meta-insights
            meta_insights = self._generate_meta_insights(
                consistency_analysis, bias_analysis, llm_meta_evaluation, meta_scores
            )
        ...
        for i in range(len(all_dimensions[0])):
            for j in range(i + 1, len(all_dimensions[0])):
                ...
                if len(dim_i_scores) >= 2:
                    ...
                    numerator = sum(
                        (dim_i_scores[k] - mean_i) * (dim_j_scores[k] - mean_j) for k in range(len(dim_i_scores))
                    )
...
"""
    ...
    def _calculate_meta_scores(
        ...
        overall_meta_score = consistency_score * consistency_weight + bias_score * bias_weight + llm_score * llm_weight
    ...
    def _assess_system_health(
        self, evaluation_history: List[Dict[str, Any]], meta_scores: Dict[str, float]
    ) -> Dict[str, Any]:
