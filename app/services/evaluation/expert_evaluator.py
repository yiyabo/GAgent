...
class ExpertRole:
    ...
    def __init__(
        self, name: str, description: str, focus_areas: List[str], weight: float = 1.0, keywords: List[str] = None
    ):
        self.name = name
...
class MultiExpertEvaluator(LLMBasedEvaluator):
    ...
    def evaluate_with_multiple_experts(
        ...
        logger.info(
            f"Multi-expert evaluation completed: {successful_evaluations}/{len(experts_to_use)} experts, consensus score: {consensus.get('overall_score', 0):.3f}"
        )
    ...
    def _generate_expert_consensus(
        ...
        numeric_fields = [key for key, value in sample_evaluation.items() if isinstance(value, (int, float))]
    ...
    def _detect_expert_disagreements(self, expert_evaluations: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _calculate_consensus_confidence(self, expert_evaluations: Dict[str, Dict[str, Any]]) -> float:
    ...
    def _create_empty_multi_expert_result(self, iteration: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "disagreements": [],
            "metadata": {"error": "empty_content", "iteration": iteration, "timestamp": datetime.now().isoformat()},
        }
    ...
    def _create_error_multi_expert_result(self, iteration: int, error_msg: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "consensus": {"overall_score": 0.0, "confidence": 0.0},
            "disagreements": [],
            "metadata": {"error": error_msg, "iteration": iteration, "timestamp": datetime.now().isoformat()},
        }
