...
1. `code` (string): The executable Python code block.
   ...
   - **NEVER use `plt.show()` or any interactive display**. Always save plots directly using `plt.savefig('results/<filename>.png')` and then `plt.close()`.
    - **For publication-quality figures: ALWAYS save in BOTH PNG (300 dpi) and PDF formats**:
      ...
      plt.close()
      ```
    - **Artifact Attribution**: Add `Generated by AI, please verify` to human-facing figures and narrative reports. Do not alter machine-readable source data, including CSV/TSV, FASTA, JSON/JSONL, BibTeX, or analysis manifests: changing their content can invalidate parsers, checksums, and downstream pipelines.
      ...
      - **Structured data**: Preserve exact schema and values; record provenance in a separate narrative report or deliverable manifest instead.
