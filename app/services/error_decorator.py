...
def database_service_error(func: Callable) -> Callable:
    ...
    return service_error_handler(
        default_error_type=DatabaseError,
        default_error_code=ErrorCode.QUERY_EXECUTION_FAILED
    )(func)
...
def llm_service_error(func: Callable) -> Callable:
    """LLM服务专用错误处理装饰器"""
    return service_error_handler(
        default_error_type=ExternalServiceError,
        default_error_code=ErrorCode.LLM_SERVICE_ERROR
    )(func)
...
def embedding_service_error(func: Callable) -> Callable:
    ...
    return service_error_handler(
        default_error_type=ExternalServiceError,
        default_error_code=ErrorCode.EMBEDDING_SERVICE_ERROR
    )(func)
...
def memory_service_error(func: Callable) -> Callable:
    """记忆服务专用错误处理装饰器"""
    return service_error_handler(
        default_error_type=SystemError,
        default_error_code=ErrorCode.MCP_SERVICE_ERROR
    )(func)


def _convert_exception(
    original_exception: Exception,
    default_error_type: Type[BaseError],
...
def _is_database_exception(exception: Exception) -> bool:
    ...
    database_indicators = [
        'sqlite', 'database', 'cursor', 'sql', 'connection',
        'integrity', 'constraint', 'foreign key', 'unique'
    ]
    
    return (
        any(indicator in exception_type for indicator in database_indicators) or
        any(indicator in error_message for indicator in database_indicators)
    )
...
def _is_network_exception(exception: Exception) -> bool:
    ...
    network_indicators = [
        'connection', 'timeout', 'http', 'requests', 'urllib',
        'socket', 'ssl', 'certificate', 'dns'
    ]
    
    return (
        any(indicator in exception_type for indicator in network_indicators) or
...
def _is_external_service_exception(exception: Exception) -> bool:
    ...
    service_indicators = [
        'api', 'client', 'service', 'token', 'auth', 'rate limit',
        'quota', 'llm', 'openai', 'anthropic', 'embedding'
    ]
    
    return (
        any(indicator in exception_type for indicator in service_indicators) or
        any(indicator in error_message for indicator in service_indicators)
    )
...
def handle_service_error(
    ...
    **kwargs
    ...
    try:
        ...
        logger.error(f"Service error in {func.__name__ if hasattr(func, '__name__') else 'function'}: {str(e)}", exc_info=True)
...
async def handle_async_service_error(
    func: Callable,
    *args,
    **kwargs
):
    ...
    try:
        ...
        logger = logging.getLogger(func.__module__ if hasattr(func, '__module__') else __name__)
        logger.error(f"Async service error in {func.__name__ if hasattr(func, '__name__') else 'function'}: {str(e)}", exc_info=True)
