...
from .recursive_decomposition import (
    TaskType, evaluate_task_complexity, determine_task_type,
    MAX_DECOMPOSITION_DEPTH, _build_decomposition_prompt
)
...
class ToolAwareTaskDecomposer:
    ...
    async def decompose_with_tool_awareness(self, task_id: int, 
                                          max_subtasks: int = 8,
        ...
        try:
            ...
            decomp_strategy = self._determine_tool_aware_strategy(
                task, tool_requirements, max_subtasks
            )
            ...
            result = await self._execute_tool_aware_decomposition(
                task_id, task, decomp_strategy, max_subtasks
            )
...
"""
    ...
    def _determine_tool_aware_strategy(self, task: Dict[str, Any], 
                                     tool_requirements: Dict[str, Any],
                                     max_subtasks: int) -> Dict[str, Any]:
    ...
    async def _execute_tool_aware_decomposition(self, task_id: int, task: Dict[str, Any],
        ...
        enhanced_prompt = self._build_tool_aware_prompt(
            task_name, task_prompt, task_type, strategy, max_subtasks
        )
        ...
        for i, subtask in enumerate(subtasks[:max_subtasks]):
            ...
            enhanced_subtask_prompt = self._enhance_subtask_prompt(
                original_prompt, strategy, i
            )
    ...
    def _build_tool_aware_prompt(self, task_name: str, task_prompt: str,
                               task_type: TaskType, strategy: Dict[str, Any],
                               max_subtasks: int) -> str:
...
async def decompose_task_with_tool_awareness(task_id: int, 
...
async def analyze_task_tool_requirements(task_id: int, 
                                       repo: Optional[TaskRepository] = None) -> Dict[str, Any]:
...
def should_use_tool_aware_decomposition(task: Dict[str, Any]) -> bool:
    ...
    prompt_indicators = [
        "搜索", "查找", "最新", "数据", "分析", "文件", "报告",
        "search", "find", "latest", "data", "analysis", "file", "report"
    ]
