...
def test_deep_think_skips_legacy_auto_decompose_for_integrated_plan_create(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _DeepThinkJobStub:
        def create_job(self, **_kwargs):
    ...
    agent = _build_deep_think_test_agent()
    agent.plan_session = _PlanSessionBindStub(plan_id=None)
    agent.session_id = "session-integrated-create"
    agent.max_history_messages = 20
    agent._dirty = False
    scheduled_reviews: list[int] = []
    ...
    def _fake_refresh(force_reload=False):  # type: ignore[no-untyped-def]
        _ = force_reload
        agent.plan_tree = SimpleNamespace(title="Integrated plan", metadata={})
        return agent.plan_tree

    agent._refresh_plan_tree = _fake_refresh
    agent._auto_decompose_plan = (
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("legacy auto-decompose should not run"))
    )
    agent._start_background_created_plan_auto_review = (
        lambda plan_id: scheduled_reviews.append(plan_id) or True
    )

    events = asyncio.run(_collect_deep_think_events(agent, "做一个同步生成的plan"))
