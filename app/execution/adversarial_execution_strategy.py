...
from ..services.evaluation_supervisor import monitor_evaluation
...
class AdversarialExecutionStrategy:
    ...
    def execute(
        ...
        result = self._execute_adversarial_iterative_loop(
            task_id, name, initial_prompt, task_context, 
            max_iterations, max_rounds, quality_threshold, improvement_threshold
        )
    ...
    def _execute_adversarial_iterative_loop(
        ...
        for iteration in range(max_iterations):
            try:
                ...
                logger.info(f"Task {task_id} iteration {iteration + 1} adversarial score: {evaluation.overall_score:.3f}")
                ...
                if evaluation.overall_score >= quality_threshold:
                    logger.info(f"Task {task_id} reached adversarial evaluation threshold: {evaluation.overall_score:.3f}")
    ...
    def _store_evaluation_history(self, task_id: int, iteration: int, content: str, evaluation):
        ...
        try:
            ...
            adversarial_metadata = {
                ...
                "robustness_score": evaluation.metadata.get('robustness_score', 0.0) if evaluation.metadata else 0.0,
                "criticism_severity": evaluation.metadata.get('criticism_severity', 'low') if evaluation.metadata else 'low',
        ...
        try:
            ...
            monitor_evaluation(
                ...
                additional_metrics={
                    ...
                    "convergence_rate": result.evaluation.metadata.get('converged', False) if result.evaluation and result.evaluation.metadata else False
                }
            )
        except Exception as e:
