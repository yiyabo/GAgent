...
from ...utils.task_path_generator import ensure_task_directory, get_task_file_path
...
class ToolEnhancedExecutor:
    ...
    def _default_report_path(
        self, task_name: str, fallback_filename: str = "report.md", task=None
    ) -> str:
        """Build a path based on task hierarchy (ROOT → COMPOSITE → ATOMIC)"""
        # 如果提供了task对象，使用层级路径生成器
        if task:
            try:
                from app.utils.task_path_generator import (
                ...
                logger.warning(
                    f"Failed to generate hierarchical path: {e}, falling back to default"
                )
    ...
    async def execute_task(
        ...
        try:
            ...
            needs_tools, tool_analysis = await self._analyze_tool_requirements(
                task_prompt, task
            )

            if needs_tools and self.tool_router:
                logger.info(
                    f"Task {task_id} requires external tools, proceeding with tool-enhanced execution"
                )

                # Phase 2: Intelligent tool routing
                routing_context = await self._build_routing_context(
                    task, context_options
                )
                logger.info(
                ...
                routing_result = await self.tool_router.route_request(
                    task_prompt, routing_context
                )
                ...
                task_type = (
                    ...
                    else (task[7] if len(task) > 7 else "atomic")
                )
                is_atomic = task_type == "atomic"

                if not output_tools and (
                    force_save
                    or is_atomic
                    or _looks_like_doc_generation(task_name or "", task_prompt or "")
                ):
                    # Build unified path using task hierarchy
                    default_path = self._default_report_path(
                        task_name or "report", custom_filename or "", task=task
                    )
                    logger.info(
                        f"🗂️ Auto-save enabled for {task_type} task → {default_path}"
                    )
                    output_tools.append({
                ...
                logger.info(
                    f"Generating content with enhanced context for task {task_id}"
                )
                if enhanced_context_options.get("tool_enhanced"):
                    logger.info(
                        f"Tool context available: {len(enhanced_context_options.get('combined', ''))} chars"
                    )
                ...
                if status in ("done", "completed"):
                    try:
                        ...
                        if corrected:
                            logger.info(
                                f"✅ Theme correction applied for task {task_id}"
                            )
                    except Exception as e:
                ...
                if output_tools and status in ("done", "completed"):
                    saved = await self._execute_post_generation_tools(
                        output_tools, task, task_id
                    )
                    if saved:
                ...
                await self._record_tool_usage(
                    task, routing_result, tool_outputs, status
                )
                ...
                logger.info(
                    f"Task {task_id} does not require tools, using standard execution with materialization"
                )
                status = base_execute_task(
    ...
    async def _execute_tool_calls(
        self, tool_calls: List[Dict[str, Any]], task
    ) -> List[Dict[str, Any]]:
        ...
        for call in tool_calls:
            try:
                ...
                if call.get("tool_name") == "file_operations":
                    ...
                    def _norm_path(p: Optional[str]) -> Optional[str]:
                        ...
                        try:
                            ...
                            raw = str(p).replace("\\", "/")
                            if raw.startswith("results/") or raw.startswith(
                                "./results/"
                            ):
                                return raw
                    ...
                    if "path" in params:
                        params["path"] = _norm_path(params.get("path")) or str(
                            Path(self.base_dir)
                        )
                ...
                logger.info(
                    f"Tool {call['tool_name']} executed successfully for task {task_id}"
                )

            except Exception as e:
                logger.error(
                    f"Tool {call['tool_name']} execution failed for task {task_id}: {e}"
                )
                ...
                if tool_name == "web_search":
                    ...
                    if isinstance(result, dict) and result.get("results"):
                        ...
                        for i, item in enumerate(result["results"][:3], 1):
                            ...
                            search_summary += (
                                f"   {item.get('snippet', 'No snippet')[:100]}...\n"
                            )
    ...
    async def _record_tool_usage(
        ...
        try:
            ...
            usage_record = {
                ...
                "tool_effectiveness": self._calculate_tool_effectiveness(
                    tool_outputs, status
                ),
            }
    ...
    def _calculate_tool_effectiveness(
        self, tool_outputs: List[Dict[str, Any]], status: str
    ) -> float:
    ...
    async def _execute_post_generation_tools(
        self, output_tools: List[Dict[str, Any]], task, task_id: int
    ) -> List[str]:
        ...
        try:
            ...
            if not generated_content:
                logger.warning(
                    f"No content available for post-generation tools in task {task_id}"
                )
                return []

            logger.info(
                f"Executing {len(output_tools)} post-generation tools for task {task_id}"
            )
                ...
                try:
                    ...
                    if tool_name == "file_operations":
                        ...
                        if parameters.get("operation") == "write":
                            ...
                            if not path_in or path_in in [
                                "report.txt",
                                "report.md",
                                "",
                            ]:
                                fallback = f"报告_{task_id}.md"
                                parameters["path"] = self._default_report_path(
                                    task.get("name", "report"), fallback
                                )
                            else:
                                ...
                                if not dn or dn in {".", "./"}:
                                    parameters["path"] = self._default_report_path(
                                        task.get("name", "report"),
                                        os.path.basename(path_in),
                                    )

                    logger.info(
                        f"Executing post-generation tool {tool_name} for task {task_id}"
                    )
                    result = await execute_tool(tool_name, **parameters)

                    if isinstance(result, dict) and result.get("success"):
                        logger.info(
                            f"✅ Post-generation tool {tool_name} executed successfully"
                        )
                        if (
                            tool_name == "file_operations"
                        ...
                        logger.warning(
                            f"⚠️ Post-generation tool {tool_name} had issues: {result}"
                        )

                except Exception as e:
                    logger.error(
                        f"❌ Post-generation tool {call['tool_name']} failed: {e}"
                    )
    ...
    async def _materialize_atomic_output(self, task) -> None:
        ...
        try:
            task_id = task.get("id") if isinstance(task, dict) else task[0]
            task_info = (
                task if isinstance(task, dict) else self.repo.get_task_info(task_id)
            )
            if not task_info:
                return
            task_type = (
                task_info.get("task_type") if isinstance(task_info, dict) else None
            )
            if task_type != "atomic":
                return
            ...
            try:
                ...
                logger.warning(
                    f"Failed to write atomic output to {file_path}: {io_err}"
                )
        except Exception as e:
    ...
    async def _maybe_assemble_upstream(self, task_id: int) -> None:
        ...
        try:
            ...
            for ch in children:
                if ch.get("task_type") == "atomic" and ch.get("status") not in {
                    "done",
                    "completed",
                }:
            ...
            try:
                CompositeAssembler(self.repo).assemble(
                    parent["id"], strategy="llm", force_real=True
                )
                logger.info(
                    "Composite %s (re-)assembled from all atomic children",
                    parent.get("id"),
                )
            except Exception as e:
                logger.warning(
                    "Composite assembly failed for %s: %s", parent.get("id"), e
                )
                return
            ...
            if comps and all(
                (c.get("task_type") != "composite" or c.get("status") == "completed")
                for c in comps
            ):
                try:
                    RootAssembler(self.repo).assemble(
                        root_parent["id"], strategy="llm", force_real=True
                    )
                    logger.info(
                        "Root %s assembled from all composite children",
                        root_parent.get("id"),
                    )
                except Exception as e:
                    logger.warning(
                        "Root assembly failed for %s: %s", root_parent.get("id"), e
                    )
        except Exception as e:
...
{{
  ...
  "suggestions": "<若偏题，给出纠正建议>"
            ...
            llm = get_llm_service()
            check_result = await llm.generate_async(
                check_prompt, temperature=0.1, max_tokens=500
            )
            ...
            if score < 7 or not on_topic:
                logger.warning(
                    f"Task {task_id} theme drift detected (score={score}), performing correction"
                )
...
重写后的内容："""
                ...
                corrected = await llm.generate_async(
                    correction_prompt, temperature=0.3, max_tokens=2000
                )

                # Update task output
                self.repo.upsert_task_output(task_id, corrected)
                logger.info(
                    f"Task {task_id} content corrected (original score: {score})"
                )
                return True
...
def create_tool_enhanced_context_options(
...
def execute_task_enhanced(task, repo=None, use_context=True, context_options=None):
    ...
    try:
        ...
        if loop and loop.is_running():
            ...
            logger.warning(
                "Cannot run async tool execution in running event loop, using base execution"
            )
...
async def execute_task_with_tools_and_evaluation(
    ...
    try:
        needs_tools, tool_analysis = await executor._analyze_tool_requirements(
            task_prompt, task
        )
    except Exception:
    ...
    if needs_tools and executor.tool_router:
        ...
        try:
            routing_result = await executor.tool_router.route_request(
                task_prompt, routing_context
            )
        except Exception as e:
    ...
    if not output_tools:
        ...
        if (
            force_save
            or is_atomic
            or _looks_like_doc_generation(task_name or "", tp_lower)
        ):
            # Create a synthetic file write action to persist the generated content with hierarchical path
            default_path = executor._default_report_path(
                task_name or "report", custom_filename or "", task=task
            )
    ...
    if output_tools and getattr(result, "status", None) in ("done", "completed"):
        try:
            saved_files = await executor._execute_post_generation_tools(
                output_tools, task, task_id
            )
        except Exception as e:
