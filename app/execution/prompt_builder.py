...
IMPROVED RESPONSE:"""
    ...
    def build_llm_revision_prompt(
        ...
        if dimensions.relevance < 0.7:
            dimension_feedback.append(f"• Relevance ({dimensions.relevance:.2f}): Better align content with the task requirements")
        if dimensions.completeness < 0.7:
            dimension_feedback.append(f"• Completeness ({dimensions.completeness:.2f}): Add missing information or expand on key points")
        ...
        if dimensions.coherence < 0.7:
            dimension_feedback.append(f"• Coherence ({dimensions.coherence:.2f}): Better organize ideas and improve flow")
        if dimensions.scientific_rigor < 0.7:
            dimension_feedback.append(f"• Scientific Rigor ({dimensions.scientific_rigor:.2f}): Strengthen methodology and evidence")
        
        dimension_text = "\\n".join(dimension_feedback) if dimension_feedback else "• Continue to maintain quality across all dimensions"
        suggestions_text = "\\n".join([f"• {s}" for s in evaluation.suggestions])
