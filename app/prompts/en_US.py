    ...
    "structured_agent": {
        "action_catalog": {
            "base_actions": [
                ...
                "- tool_operation: phagescope (PhageScope phage analyses; action in ping/input_check/submit/task_list/task_detail/task_log/result/quality/download/save_all/batch_submit/batch_reconcile/batch_retry; requires `action`, optional phageid/userid/modulelist/taskid/result_kind/download_path/save_path/session_id/phage_ids/batch_id/strategy/target_task_id. IMPORTANT: submit is async and should return immediately; do not wait for completion in the same turn. For submit, `modulelist` must contain submit modules only; `proteins` is an annotation-derived result/output, not a submit module.)",
                ...
                "- tool_operation: deliverable_submit (promote specific files into the session Deliverables sidebar for paper/submission; requires `artifacts` as a list of {path, module} where module is one of code|image_tabular|paper|refs|docs; optional `publish`=false to skip this call. When the server sets DELIVERABLES_INGEST_MODE=explicit, claude_code and other tools no longer auto-mirror outputs into Deliverables—you must call this tool when the user wants figures/code/refs published there.)",
        ...
        "guidelines": {
            "common_rules": [
                ...
                "Before invoking heavy tools such as `claude_code`, organize project-level or multi-step requests into a structured plan first; create or refine tasks automatically when needed.",
                ...
                "IMPORTANT: For accession-based FASTA downloads, use `sequence_fetch` first. For FASTA/FASTQ/sequence analysis, use `bio_tools` first (start with operation='help' when uncertain, and prefer `seqkit stats` for quick diagnostics). If the user provides inline sequence text, pass it via `sequence_text` so bio_tools converts it safely to FASTA. Do not use `claude_code` for input-format conversion fallback when bio_tools input preparation fails or when sequence_fetch fails.",
