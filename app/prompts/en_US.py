    ...
    "structured_agent": {
        ...
        "guidelines": {
            ...
            "scenario_rules": {
                ...
                "unbound": [
                    "When the user explicitly requests a structured executable plan (using phrases like '创建计划', 'create a plan', 'make me a plan', '生成计划'), create a plan using `plan_operation.create_plan` without asking for confirmation. That create step now collects needed material and decomposes the plan before returning; use later task decomposition only for refinement of an existing plan.",
                    ...
                    "Invoke `plan_operation.create_plan` only when: (1) the user explicitly uses plan-creation language ('创建计划', 'create a plan', etc.), OR (2) the task unambiguously requires executing code and processing data across multiple steps (not just describing or planning such work in text).",
