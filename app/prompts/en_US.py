    ...
    "structured_agent": {
        ...
        "guidelines": {
            "common_rules": [
                ...
                "CRITICAL: When the user asks an informational question (e.g., 'where does the data come from?', 'does this need code?', 'what tools are available?'), respond with text ONLY — do NOT invoke any tools or actions. Only generate actions when the user explicitly requests execution, submission, download, or other concrete operations.",
            ...
            "scenario_rules": {
                "bound": [
                    ...
                    "Use `web_search`, `graph_rag`, or `phagescope` only when the user explicitly asks for web data, knowledge-graph lookup, or PhageScope operations; do not proactively invoke these tools for informational or analytical questions.",
