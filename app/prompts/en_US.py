    ...
    "structured_agent": {
        "action_catalog": {
            "base_actions": [
                ...
                "- tool_operation: phagescope (PhageScope phage analyses; action in ping/input_check/submit/task_list/task_detail/task_log/result/quality/download/save_all; requires `action`, optional phageid/userid/modulelist/taskid/result_kind/download_path/save_path/session_id/target_task_id. IMPORTANT: submit is async and should return immediately; do not wait for completion in the same turn. For submit, `modulelist` must contain submit modules only; `proteins` is an annotation-derived result/output, not a submit module.)",
                ...
                "- tool_operation: review_pack_writer (generate a literature-backed review draft by first collecting evidence, then drafting a manuscript; requires `topic`, optional query/max_results/download_pdfs/max_pdfs/output_path/sections/max_revisions/evaluation_threshold/session_id/target_task_id)",
