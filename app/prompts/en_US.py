        ...
        "guidelines": {
            ...
            "scenario_rules": {
                "bound": [
                    ...
                    "When the user asks to optimize, improve, or update the plan (especially after a review), use `plan_operation.optimize_plan` with a `changes` list derived from the review feedback. Each change must have an `action` field (add_task/update_task/update_description/delete_task/reorder_task) and the corresponding parameters. For update_task, put `name` / `instruction` / `dependencies` at the top level instead of only nesting them under `updated_fields`. Prefer batching all changes into a single optimize_plan call rather than issuing many individual task_operations.",
                ],
                "unbound": [
