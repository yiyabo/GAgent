#!/bin/bash
# Build the gagent-qwen-code-runtime Docker image.
# This image extends gagent-python-runtime with Node.js and the Qwen Code CLI.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
IMAGE_TAG="${QWEN_CODE_DOCKER_IMAGE:-gagent-qwen-code-runtime:latest}"
BASE_IMAGE="${QWEN_CODE_BASE_IMAGE:-gagent-python-runtime:latest}"
...
build_args=(
  ...
  -t "${IMAGE_TAG}"
  -f "${ROOT_DIR}/docker/qwen_code_runtime/Dockerfile"
...
echo "Building ${IMAGE_TAG} (base: ${BASE_IMAGE}) for ${DOCKER_PLATFORM}"
cd "${ROOT_DIR}"
docker buildx build "${build_args[@]}" .

echo ""
echo "✅  ${IMAGE_TAG} built successfully"
echo "   node $(docker run --rm "${IMAGE_TAG}" node --version)"
echo "   qwen $(docker run --rm "${IMAGE_TAG}" qwen --version)"
